...
![CI](https://github.com/patrtorg/ab-iure-ad/workflows/CI/badge.svg)
[![NPM version](https://img.shields.io/npm/v/@patrtorg/ab-iure-ad.svg?style=flat)](https://www.npmjs.com/package/@patrtorg/ab-iure-ad)
[![js-standard-style](https://img.shields.io/badge/code%20style-standard-brightgreen.svg?style=flat)](https://standardjs.com/)
[![NPM downloads](https://img.shields.io/npm/dm/@patrtorg/ab-iure-ad.svg?style=flat)](https://www.npmjs.com/package/@patrtorg/ab-iure-ad)


__@patrtorg/ab-iure-ad__ is significantly faster than `JSON.stringify()` for small payloads.
...
@patrtorg/ab-iure-ad requires a [JSON Schema Draft 7](https://json-schema.org/specification-links.html#draft-7) input to generate a fast `stringify` function.
...
@patrtorg/ab-iure-ad array default x 6,243 ops/sec ±0.14% (98 runs sampled)
@patrtorg/ab-iure-ad array json-stringify x 6,261 ops/sec ±0.30% (99 runs sampled)
...
@patrtorg/ab-iure-ad large array default x 99.96 ops/sec ±0.22% (74 runs sampled)
@patrtorg/ab-iure-ad large array json-stringify x 248 ops/sec ±0.07% (90 runs sampled)
...
@patrtorg/ab-iure-ad long string x 15,979 ops/sec ±0.09% (96 runs sampled)
...
@patrtorg/ab-iure-ad short string x 30,585,790 ops/sec ±0.27% (97 runs sampled)
...
@patrtorg/ab-iure-ad obj x 6,866,434 ops/sec ±0.11% (100 runs sampled)
...
@patrtorg/ab-iure-ad date format x 1,836,188 ops/sec ±0.12% (99 runs sampled)
...
<a name="example"></a>
Try it out on RunKit: <a href="https://runkit.com/npm/@patrtorg/ab-iure-ad">https://runkit.com/npm/@patrtorg/ab-iure-ad</a>
## Example

```js
const fastJson = require('@patrtorg/ab-iure-ad')
...
Optionally, you may provide to `@patrtorg/ab-iure-ad` an option object as second parameter:
...
```
If the object to stringify is missing the required field(s), `@patrtorg/ab-iure-ad` will throw an error.

<a name="missingFields"></a>
#### Missing fields
If a field *is present* in the schema (and is not required) but it *is not present* in the object to stringify, `@patrtorg/ab-iure-ad` will not write it in the final string.
Example:
```javascript
const stringify = fastJson({
  title: 'Example Schema',
...
<a name="defaults"></a>
#### Defaults
`@patrtorg/ab-iure-ad` supports `default` jsonschema key in order to serialize a value
...
#### Pattern properties
`@patrtorg/ab-iure-ad` supports pattern properties as defined by JSON schema.
...
`@patrtorg/ab-iure-ad` supports additional properties as defined by JSON schema.
...
`@patrtorg/ab-iure-ad` supports the **anyOf** and **oneOf** keywords as defined by JSON schema. Both must be an array of valid JSON schemas. The different schemas will be tested in the specified order. The more schemas `stringify` has to try before finding a match, the slower it will be.
...
#### If/then/else
`@patrtorg/ab-iure-ad` supports `if/then/else` jsonschema feature. See [ajv documentation](https://ajv.js.org/keywords.html#ifthenelse).
...
```
If you need to use an external definition, you can pass it as an option to `@patrtorg/ab-iure-ad`.
Example:
```javascript
const schema = {
  title: 'Example Schema',
...
`@patrtorg/ab-iure-ad` to handle arrays starts increasing exponentially, leading
...
@patrtorg/ab-iure-ad large array default x 48.72 ops/sec ±4.92% (48 runs sampled)
@patrtorg/ab-iure-ad large array json-stringify x 157 ops/sec ±0.76% (86 runs sampled)
...
To achieve low cost and high performance redaction `@patrtorg/ab-iure-ad`
...
Users are responsible for sending trusted data. `@patrtorg/ab-iure-ad` guarantees that you will get
...
itself. You need to have `@patrtorg/ab-iure-ad` installed for the standalone code to work.
